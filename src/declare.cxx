...
void declaration(bparser::node& sprite, bparser::node& code) {
	...
	if (code.value == "name") {
		if (code.size() != 1) {
			throw std::runtime_error("Wrong number of parameters (Expected 1)");
		}
		...
		if (!checknum(code[0].value)) {
			throw std::runtime_error("Invalid value (Expected a number)");
		}
		...
		if (code[0].value != "true" && code[0].value != "false") {
			throw std::runtime_error("Invalid value (Expected \"true\" or \"false\")");
		}
		...
		if (code[0].value != "all around" && code[0].value != "left-right" && code[0].value != "don't rotate") {
			throw std::runtime_error("Invalid value (Expected \"all around\", \"left-right\", or \"don't rotate\")");
		}
		sprite.find("rotationStyle")[0].value = code[0].value;
	}
	else {
		std::ostringstream error;
		error << "Unknown declaration \"" << code.value << "\"";
